"""
CIFAR-100 İçin Özelleştirilmiş Mini Vision Transformer (MiniViT)
----------------------------------------------------------------
Yama Gömülmesi, [CLS] Token, 1D Pozisyonel Gömülmeler, Pre-LN Encoder Blokları
ve CIFAR-100 için 100 Sınıflı MLP Sınıflandırma Kafası.
...
class YamaGomulmeKatmani(nn.Module):
    """2D Görseli Patch Dizisine Dönüştürür: (B, C, H, W) -> (B, N, D)"""
...
class CokKafaliOzDikkat(nn.Module):
    ...
    def __init__(self, model_boyutu: int = 64, kafa_sayisi: int = 4, dropout_orani: float = 0.1):
        super().__init__()
...
class BeslemeliIleriAg(nn.Module):
    """GELU Aktivasyonlu MLP Katmanı"""
    def __init__(self, model_boyutu: int = 64, genisleme: int = 4, dropout_orani: float = 0.1):
        super().__init__()
...
class TransformerEncoderBlogu(nn.Module):
    ...
    def __init__(self, model_boyutu: int = 64, kafa_sayisi: int = 4, genisleme: int = 4, dropout: float = 0.1):
        super().__init__()
...
class MiniVisionTransformer(nn.Module):
    """
    CIFAR-100 için Optimize Edilmiş Mini Vision Transformer
    """
    def __init__(
        self,
        ...
        # 3. 1D Pozisyonel Gömülmeler (1 + N yama)
        ...
        # 5. Çıkış Normalizasyonu ve Sınıflandırma Kafası
